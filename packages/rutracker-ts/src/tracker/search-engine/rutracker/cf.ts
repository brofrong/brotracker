...
/** Byparr default; FlareSolverr-compatible `/v1` also works. */
export const DEFAULT_CF_SOLVER_URL = "http://localhost:8191/v1";
...
const solverCookieSchema = z.object({
	...
	/** Unix seconds; -1 / missing = session cookie. */
...
export type CfClearanceOptions = {
	fileStore: FileStore;
	/**
...
export function normalizeSolverUrl(url: string): string {
	const trimmed = url.trim().replace(/\/+$/, "");
	return trimmed.endsWith("/v1") ? trimmed : `${trimmed}/v1`;
}

export function toStoredCookie(
	cookie: z.infer<typeof solverCookieSchema>,
): StoredCookie {
	const expires =
		cookie.expires == null || cookie.expires < 0 ? null : cookie.expires;
	return {
		name: cookie.name,
		value: cookie.value,
...
function isRutrackerCookieDomain(domain: string | undefined): boolean {
	if (!domain) return false;
	const d = domain.replace(/^\./, "").toLowerCase();
	return d === "rutracker.org" || d.endsWith(".rutracker.org");
}

/**
 * Prefer cf_clearance issued for rutracker.org.
 * Byparr sometimes also returns a clearance cookie for `.cloudflare.com`
 * (challenge iframe) — that one must not be used for HTTP replay.
 */
...
/**
 * Asks Byparr (or FlareSolverr) to open Rutracker, solve Cloudflare, and
 * persist cf_clearance + User-Agent for later HTTP replay.
 */
export async function acquireCfClearance(
	options: CfClearanceOptions,
	...
	const challengeUrl =
		options.challengeUrl ?? `${RUTRACKER_URL}/forum/login.php`;
	/** Byparr: seconds. FlareSolverr: milliseconds (`maxTimeout`). Send both. */
	...
	try {
		...
		const cfClearance = extractCfClearance(body.solution.cookies);
		if (!cfClearance) {
			return err(
				new Error(
		...
		console.log("[rutracker] cf_clearance saved to", options.fileStore.path);
