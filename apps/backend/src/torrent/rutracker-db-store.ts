...
/** Postgres-backed FileStore for RuTracker session/CF cookies. */
export function createRutrackerDbStore(): FileStore {
	async function read(): Promise<Result<FileStoreData, Error>> {
		try {
			const rows = await db
				.select({ data: rutrackerStore.data })
				.from(rutrackerStore)
				.where(eq(rutrackerStore.id, STORE_ID))
				.limit(1);
			const row = rows[0];
			if (!row) {
			...
			if (!parsed.success) {
				return err(
					new Error(`Invalid rutracker store: ${parsed.error.message}`),
				);
			}
			return ok(parsed.data);
		} catch (error) {
			return err(new Error(`Failed to read rutracker store: ${error}`));
		}
	}
	...
	async function write(data: FileStoreData): Promise<Result<void, Error>> {
		try {
			...
			const now = new Date();
			await db
				.insert(rutrackerStore)
				.values({ id: STORE_ID, data: parsed, updatedAt: now })
				.onConflictDoUpdate({
					target: rutrackerStore.id,
					set: { data: parsed, updatedAt: now },
				});
			return ok(undefined);
		} catch (error) {
			return err(new Error(`Failed to write rutracker store: ${error}`));
		}
	}
