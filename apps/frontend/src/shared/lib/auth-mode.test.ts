...
describe("fetchAuthMode", () => {
	test("requests /api/auth/mode and returns JSON", async () => {
		...
		await expect(
			fetchAuthMode("http://localhost:3101", fetchImpl),
		).resolves.toEqual({
			mode: "local",
			registrationOpen: true,
		});
		expect(fetchImpl).toHaveBeenCalledWith(
			"http://localhost:3101/api/auth/mode",
			{
				credentials: "include",
			},
		);
	...
	test("throws on non-OK response", async () => {
		...
		await expect(fetchAuthMode("", fetchImpl)).rejects.toThrow(
			/Failed to fetch auth mode/,
		);
	});
});
